...
for mm in range(nmodel):
    ...
    for ww in range(nwave):
        spectra_int_med[ww]=np.median(integrated.spec[ww,0,:])
        spectra_int_05[ww]=np.percentile(integrated.spec[ww,0,:],05)
        spectra_int_25[ww]=np.percentile(integrated.spec[ww,0,:],25)
        spectra_int_75[ww]=np.percentile(integrated.spec[ww,0,:],75)
        spectra_int_95[ww]=np.percentile(integrated.spec[ww,0,:],95)

    #store medians
    medians.append(spectra_int_med)    
    wavmed.append(integrated.wl)    
    ...
    #5-95%
    plt.fill_between(integrated.wl,spectra_int_05,spectra_int_95,facecolor='red', alpha=0.5)
    plt.plot(integrated.wl,spectra_int_05,color='red', linewidth=3)
    plt.plot(integrated.wl,spectra_int_95,color='red', linewidth=3)
    
    #25-75%
    plt.fill_between(integrated.wl,spectra_int_25,spectra_int_75,facecolor='blue', alpha=0.6)
    plt.plot(integrated.wl,spectra_int_25,color='blue', linewidth=3)
    plt.plot(integrated.wl,spectra_int_75,color='blue', linewidth=3)

    #median
    plt.plot(integrated.wl,spectra_int_med,color='black', linewidth=3 )
