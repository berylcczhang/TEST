...
def read_cluster_cloudyparams(model_name, output_dir=None, fmt=None,
    ...
    if fname.endswith('.txt'):
        ...
        while True:
            ...
            hden.extend(data_list[1::nfield])
            r0.extend(data_list[2::nfield])
            rS.extend(data_list[3::nfield])
            QH0.extend(data_list[4::nfield])
            covFac.extend(data_list[5::nfield])
            U.extend(data_list[6::nfield])
    ...
    out = cloudyparams_type(
        cluster_id, trial, time, hden, r0, rS, QH0, covFac, U, Omega)
